...
void FindIntermediateColl (int r, uint32_t DjX0i, uint32_t Djstepsi,
  		  ...
  		  if (Djstepsi<Djstepsk){
               		 ...
               		 for (int d=0; d<diff; d++) {
                       		 tempReach[d+1]=baseFunct(tempReach[d],r, MAXP);
  			  //printf("%d step %d \n", d, tempReach[d]);
               		 }
...
while (nCollisFinal < 1){
   ...
   for (int i=0; i<n_per_proc; i++) nDPj+=nDPaux[i];
	...
	//printf("num interr coll of ietration %d \n", nCollisIr);
	//frees on device
	free(tempA);
	free(tempB);

	//printf("nCollisTot = %d , nCollisIr = %d & their sum = %d \n ", nCollisTot, nCollisIr, nCollisTot+nCollisIr);
